-- MVP match model: two distinct memories owned by the same member compete.
-- Member-to-member matching, participant registration, photos, and connection ranking are excluded.
...
CREATE INDEX idx_members_name_id ON members (name, id);
