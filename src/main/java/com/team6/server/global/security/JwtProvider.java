...
@Component public class JwtProvider {
 ...
 public Claims parse(String token){try{return Jwts.parser().verifyWith(key).build().parseSignedClaims(token).getPayload();}catch(JwtException|IllegalArgumentException e){throw new com.team6.server.global.exception.BusinessException(com.team6.server.global.exception.ErrorCode.INVALID_TOKEN);}}
